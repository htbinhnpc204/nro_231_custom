...
public class GameScr : mScreen, IChatable
{
    ...
    public void createMenu(string[] menu, Npc npc)
    {
        ...
        {
            if (menu[i].Contains("VIP") && Code.isChangeSpecialSkill)
            {
                Service.gI().confirmMenu((short)npc.template.npcTemplateId, (sbyte)i);
            }
            if (menu[i].Contains("Nâng cấp") && Code.StartUpgrade)
            {
                Service.gI().confirmMenu((short)npc.template.npcTemplateId, (sbyte)i);
                return;
            }
            ...
            if (GameCanvas.keyAsciiPress != 0)
            {
                if (mobCapcha == null)
                {
                    if (TField.isQwerty)
                    {
                        ...
                        else if (GameCanvas.keyAsciiPress == 'g')
                        {
                            Char mychar = Char.myCharz();
                            mychar.cx = GetXFocus(mychar);
                            mychar.cy = GetYFocus(mychar);
                        }
                        else if (GameCanvas.keyAsciiPress == '<')
                        {
                            Time.timeScale -= 0.5f;
                            Code.Paint("Speed Game: " + Time.timeScale);
                        }
                        else if (GameCanvas.keyAsciiPress == '>')
                        {
                            Time.timeScale += 0.5f;
                        ...
                        else if (GameCanvas.keyAsciiPress == 'b')
                        {
                            Code.UseItem(194);
                        }
                        ...
                        else if (GameCanvas.keyAsciiPress == 't')
                        {
                            if (!Code.canAutoPlay)
                            {
                                MyVector myVector = new MyVector();
                                myVector.addElement(new Command("Tất cả", 20401, string.Empty));
                                foreach (var item in vMob.a.Cast<Mob>().GroupBy(x => x.getTemplate().name).Select(x => x.FirstOrDefault()).ToList())
                                {
                                    myVector.addElement(new Command(item.getTemplate().name, 20401, item.getTemplate().name));
                                }
                                GameCanvas.menu.startAt(myVector, 0);
                            }
                            else
                            {
                                Code.canAutoPlay = false;
                            }
                        }
                        else if (GameCanvas.keyAsciiPress == 'z')
                        {
                            Code.isKillBoss = !Code.isKillBoss;
                        }
                        AALoadMap.KeyPress();
                    }
    ...
    private int GetYFocus(Char mychar)
    {
        return mychar.charFocus != null ? mychar.charFocus.cy : mychar.npcFocus != null ? mychar.npcFocus.cy : mychar.mobFocus != null ? mychar.mobFocus.getY() : mychar.itemFocus != null ? mychar.itemFocus.y : mychar.cy;
    }

    private int GetXFocus(Char mychar)
    {
        return mychar.charFocus != null ? mychar.charFocus.cx : mychar.npcFocus != null ? mychar.npcFocus.cx : mychar.mobFocus != null ? mychar.mobFocus.getX() : mychar.itemFocus != null ? mychar.itemFocus.x : mychar.cx;
    }
    ...
    {
        canAutoPlay = isAutoPlay = Code.canAutoPlay;
        Char.myCharz().cspeed = Code.cSpeed;
        if (GameCanvas.gameTick % 20 == 0 && Code.SaleTrash)
        {
            for (int i = Char.myCharz().arrItemBag.Length - 1; i >= 0; i--)
            {
                var item = Char.myCharz().arrItemBag[i];
                if (item != null && ((!item.isSPLorSKH && item.template.type >= 0 && item.template.type <= 4) || (Char.myCharz().xu < 200 * 1000 * 1000 && item.template.id == 457)))
                {
        ...
        {
            Code.AttackChar(Char.myCharz());
        }
        if (GameCanvas.gameTick % 10 == 0 && Code.isAutoAdd)
        {
            if (Code.getPotential())
            {
                Service.gI().upPotential(Code.Potential, 100);
            }
        }
        if (Code.isOpenCSKB && !Code.isUsing && GameCanvas.gameTick % 500 == 0)
        {
            Code.openCSKB();
        }
        if (Code.isAutoTake && GameCanvas.gameTick % 20 == 0)
        {
            try
            {
        ...
        if (Code.objKMT != null && GameCanvas.gameTick % 20 == 0)
        {
            Char mychar = Char.myCharz();
            if (Code.objKMT is Char @char)
            {
                Char focus = findCharInMap(@char.cName);
                if (focus != null)
                {
        ...
        if (!Char.myCharz().isDie 
            && Char.myPetz().cHP > 0 
            && (Char.myPetz().cMP < Char.myPetz().cMPFull * 0.05
                || Char.myPetz().cHP < Char.myPetz().cHPFull * 0.25 
                || Char.myPetz().cStamina < Char.myPetz().cMaxStamina * 0.1)
            && Char.myPetz().petStatus == 2
            )
        {
            doUseHP();
        }
        ...
        if (isAutoPlay && GameCanvas.gameTick % 5 == 0)
        {
            autoPlay();
        }
        updateXoSo();
    ...
    public override void paint(mGraphics g)
    {
        ...
        if (!isPaintOther)
        {
            ...
            foreach (var item in vCharInMap.a.Cast<Char>().OrderBy(x => x.cHPFull).Select((a, b) => new { a, b }))
            {
                if (item.a.cTypePk == 5 && item.a != Char.myCharz())
                {
                    string s = item.a.cName + ": " + NinjaUtil.getMoneys(item.a.cHP) + "/" + NinjaUtil.getMoneys(item.a.cHPFull);
                    bool isSuperBroly = item.a.cName.ToLower().Contains("super") || item.a.cName.ToLower().Contains("super broly");

                    //if (isSuperBroly)
                        //Code.Paint(isSuperBroly.ToString());

                    if (item.a.cHP > (isSuperBroly ? item.a.cHPFull * 0.1 : Code.getMyDame()))
                    {
                        mFont.bigNumber_red.drawString(g, s, GameCanvas.w / 2 - 30, 25 - (yboss -= 10), mFont.CENTER);
                    }
                    else
                    {
                        if (Code.isKillBoss && item.a.cHP > 0) Code.doKillBoss(item.a);
                        mFont.bigNumber_While.drawString(g, s, GameCanvas.w / 2 - 30, 25 - (yboss -= 10), mFont.CENTER);
                    }
                    g.setColor(Color.red);
                    if (!(Char.myCharz().cx == item.a.cx && Char.myCharz().cy == item.a.cy))
                        g.drawLine(Char.myCharz().cx - cmx, Char.myCharz().cy - cmy, item.a.cx - cmx, item.a.cy - cmy);
                }
                else
                {
                    if (string.IsNullOrEmpty(item.a.cName) || item.a.cName.Contains("$") || item.a.cName.Contains("#") || item.a.isPet)
                        continue;
                    (item.a.cTypePk == 3 ? mFont.tahoma_7b_red : mFont.tahoma_7_yellow)
                        .drawString(g,
                            $"{item.b} - {item.a.cName} - {NinjaUtil.getMoneys(item.a.cHP)}/{NinjaUtil.getMoneys(item.a.cHPFull)} ({item.a.getHPPercent():#.##}%)",
                            GameCanvas.w - 15, yy += 10, mFont.RIGHT, mFont.tahoma_7_grey);
                }
            }
            ...
            if (Code.isKillBoss)
            {
                mFont.bigNumber_green.drawString(g, "MyDame: " + NinjaUtil.getMoneys(Code.getMyDame()), GameCanvas.w / 2 - 30, 15, mFont.CENTER);
            }
            foreach (var item in Code.vBossSpawn)
            {
                var bossName = item.name.Replace("BOSS ", "");

                var seconds = (int)DateTime.Now.Subtract(item.createAt).TotalSeconds;

                (TileMap.mapID == item.mapID ? mFont.tahoma_7b_red : mFont.tahoma_7_yellow)
                    .drawString(g,
                        $"{bossName} - {item.mapName}[{item.mapID}] - {(seconds < 60 ? seconds + "s" : $"{seconds / 60}:{seconds % 60}")}",
                        GameCanvas.w - 15, y += 10, mFont.RIGHT, mFont.tahoma_7_grey);
            ...
            if (Char.myCharz().havePet && Char.myPetz() != null)
            {
                Char char5 = Char.myPetz();
                mFont.tahoma_7_yellow.drawString(g, "===>Đệ tử<=== ", 30, GameCanvas.h - 140, 0, mFont.tahoma_7_grey);
                mFont.tahoma_7_yellow.drawString(g, "SM: " + NinjaUtil.getMoneys(char5.cPower), 30, GameCanvas.h - 130, 0, mFont.tahoma_7_grey);
                mFont.tahoma_7_yellow.drawString(g, "TN: " + NinjaUtil.getMoneys(char5.cTiemNang), 30, GameCanvas.h - 120, 0, mFont.tahoma_7_grey);
                mFont.tahoma_7_yellow.drawString(g, "SĐ: " + char5.getString(7), 30, GameCanvas.h - 110, 0, mFont.tahoma_7_grey);
                mFont.tahoma_7_yellow.drawString(g, "HP: " + char5.getString(0) + "/" + char5.getString(2) + "(" + char5.getHPPercent().ToString("#.#") + "%)", 30, GameCanvas.h - 100, 0, mFont.tahoma_7_grey);
            }
    ...
    public void onChatFromMe(string text, string to)
    {
        ...
        else if (!text.Equals(string.Empty) && Code.chatMod(text))
        {
    ...
    public void openUIZone(Message message)
    {
        ...
        if (Code.isOpenUIZone)
        {
            Code.isOpenUIZone = false;
    ...
    internal static Char FindBOSSInMap(string name)
    {
        for (int i = 0; i < vCharInMap.size(); i++)
        {
            Char @char = (Char)vCharInMap.elementAt(i);
            if ((@char.cName.ToLower().Equals(name.ToLower()) || @char.cName.ToLower().Contains(name.ToLower()))
                && @char.cName.Contains(" ") && (@char.cTypePk == 3 || @char.cTypePk == 5))
            {
                return @char;
            }
        }
