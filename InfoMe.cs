...
public class InfoMe
{
	...
	public void addInfo(string s, int Type)
	{
        if (Code.isUnlockBag)
        {
            Code.isUnlockBag = !s.Contains("giới hạn");
        }
        if (Code.isAutoAdd)
        {
            Code.isAutoAdd = !s.Contains("giới hạn");
        }
		if (Code.isChangeSpecialSkill && (Regex.IsMatch(s, "(?<=\\+)(.*?)(?=\\%)") || s.ToLower().Contains("chí mạng")))
		{
			if (s.ToLower().Contains("chí mạng"))
			{
				Service.gI().speacialSkill(0);
				return;
			}
			string[] array = File.ReadAllText("noitai.txt").Split('|');
            int num = int.Parse(Regex.Match(s, "(?<=\\+)(.*?)(?=\\%)").Value);
            Code.isChangeSpecialSkill = !s.ToLower().Contains(array[0]) || num < int.Parse(array[1]);
            if (s.ToLower().Contains(array[0]))
            {
                Code.Paint(num.ToString());
            }
            if (Code.isChangeSpecialSkill)
            {
                Service.gI().speacialSkill(0);
            }
            return;
        }
        if ((s.ToLower().Contains("giây") || s.ToLower().Contains("phút")) && s.ToLower().Contains("bảo trì"))
        {
            //GameCanvas.loginScr.backToServerScr();
        }
