                ...
                case 93:
                    {
                        ...
                        try
                        {
                            ...
                            {
                                var arr = str.Replace(" vừa xuất hiện tại ", "|").Split('|');
                                var map = TileMap.mapNames.Select((value, index) => new { Id = index, Name = value });
                                var id = map.FirstOrDefault(x => x.Name.Equals(arr[1].Trim())).Id;
                                BossSpawn boss = new BossSpawn
                                {
                                ...
                                if (Code.vBossSpawn.Count >= 5)
                                    Code.vBossSpawn.RemoveAt(0);

                                Code.vBossSpawn.Add(boss);
                            }
