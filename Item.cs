...
public class Item
{
	...
	public int getStar=> itemOption?.Where(x => !string.IsNullOrEmpty(x.getOptionString()) && x.optionTemplate.id == 102).FirstOrDefault()?.param ?? 0;

    public int getStarAvailable => itemOption?.Where(x => !string.IsNullOrEmpty(x.getOptionString()) && x.optionTemplate.id == 107).FirstOrDefault()?.param ?? 0;

    public bool isSPLorSKH => itemOption?.Where(x => !string.IsNullOrEmpty(x.getOptionString()) && x.optionTemplate.name.StartsWith("$") || x.optionTemplate.id == 102 || x.optionTemplate.id == 107).FirstOrDefault() != null;

    public bool isSetPicolo => itemOption?.Where(x => !string.IsNullOrEmpty(x.getOptionString()) && x.optionTemplate.name.ToLower().Contains("sappo")).Count() > 0;
