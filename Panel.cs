...
public class Panel : IActionListener, IChatable
{
    ...
    private void paintPetInventory(mGraphics g)
    {
        ...
        for (int i = 0; i < arrItemBody.Length + arrPetSkill.Length; i++)
        {
            ...
            if (item != null)
            {
                ...
                mFont2.drawString(g, item.template.name + text + (((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32) ? $"[{item.getStar}]" : string.Empty), num3 + 5, num4 + 1, 0);
                string text2 = string.Empty;
                if (item.itemOption != null)
                {
    ...
    private void paintBox(mGraphics g)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < currentListLength; i++)
            {
                ...
                mFont2.drawString(g, item.template.name + text + (((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32) ? $"[{item.getStar}]" : string.Empty), num2 + 5, num3 + 1, 0);
                string text2 = string.Empty;
                if (item.itemOption != null)
                {
                    if (item.itemOption.Length > 0 && item.itemOption[0] != null)
    ...
    private void paintCombine(mGraphics g)
    {
        ...
        for (int j = 0; j < vItemCombine.size() + 1; j++)
        {
            ...
            mFont2.drawString(g, item.template.name + text + (((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32) ? $"[{item.getStarAvailable}]" : string.Empty), num + 5, num2 + 1, 0);
            string text2 = string.Empty;
            if (item.itemOption != null)
            {
                if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
    ...
    private void paintInventory(mGraphics g)
    {
        ...
        try
        {
            for (int j = 1; j < currentListLength; j++)
            {
                ...
                mFont2.drawString(g, item.template.name + text + (((item.template.type >= 0 && item.template.type <= 4) || item.template.type == 32) ? $"[{item.getStarAvailable}]" : string.Empty), num4 + 5, num5 + 1, 0);
                string text2 = string.Empty;
                if (item.itemOption != null)
                {
                    if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
                    ...
                    mFont3.drawString(g, text2, num4 + 5, num5 + 11, mFont.LEFT); ;
                }
    ...
    private void doFireCombine()
    {
        if (currentTabIndex == 0)
        {
            ...
            myVector.addElement(new Command("Nâng sao", this, 6004, currItem));
            if (currItem != null)
            {
                GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
                addItemDetail(currItem);
        ...
        if (currentTabIndex < currentTabName.Length - ((GameCanvas.panel2 == null) ? 1 : 0) && type != 17)
        {
            ...
            if (currItem != null)
            {
                ...
                else if (typeShop != 2)
                {
                    ...
                    myVector.addElement(new Command(mResources.BUYS, this, 3006, currItem));
                }
                else
                {
            ...
            if (!GetInventorySelect_isbody(selected, newSelected, Char.myCharz().arrItemBody))
            {
                ...
                if (item != null)
                {
                    ...
                    if (GameCanvas.panel.type == 12)
                    {
                        ...
                        //myVector.addElement(new Command("Ép đá", this, 6001, currItem));
                        myVector.addElement(new Command("Nâng sao", this, 6002, currItem));
                    }
        ...
        if (!Char.myCharz().havePet)
        {
            switch (selected)
            {
                ...
                case 4:
                    ...
                    {
                        Service.gI().openUIZone(); Code.isOpenUIZone = true;
                    }
                    break;
                case 5:
                    GameCanvas.endDlg();
    ...
    public void perform(int idAction, object p)
    {
        if (idAction == 30005)
        {
            ...
            {
                for (int num2 = Char.myCharz().arrItemShop[currentTabIndex].Length - 1; num2 >= 0; num2--)
                {
                    Item item28 = Char.myCharz().arrItemShop[currentTabIndex][num2];
                    if (item28 != null)
                    {
                        Service.gI().buyItem(1, num2, 0);
                        Thread.Sleep(50);
                    }
                }
            });
        ...
        if (idAction == 30006)
        {
            ...
            {
                for (int num = Char.myCharz().arrItemShop[currentTabIndex].Length - 1; num >= 0; num--)
                {
                    Item item27 = Char.myCharz().arrItemShop[currentTabIndex][num];
                    if (item27 != null && (item27.template.id == 381 || item27.template.id == 382 || item27.template.id == 384))
                    {
        ...
        if (idAction == 6004)
        {
            ...
            chatTField.strChat = "Nhập số sao";
            chatTField.tfChat.name = mResources.input_quantity;
            chatTField.to = string.Empty;
            chatTField.isShow = true;
            chatTField.tfChat.setIputType(TField.INPUT_TYPE_NUMERIC);
        ...
        {
            Item item2 = (Item)p;
            Code.vItemCombine.removeAllElements();
            Code.vItemCombine.addElement(item2);
            MyVector myVector = new MyVector();
            for (int i = 1; i < 13; i++)
            {
                myVector.addElement(new Command($"{i} sao", 6003, i));
            }
        ...
        if (idAction == 3006)
        {
            ...
            chatTField.strChat = mResources.BUYS;
            chatTField.tfChat.name = mResources.input_quantity;
    ...
    {
        Code.Paint(chatTField.strChat);
        if (text.Equals(string.Empty) || text == null)
        {
            chatTField.isShow = false;
            return;
        }
        if (chatTField.strChat.Equals("Nhập số sao"))
        {
            int num = 0;
            try
            {
            ...
            new Thread(() => {
                var item = vItemCombine.a.Cast<Item>().FirstOrDefault();
                Code.StartUpgrade = true;

                while (item != null && item.getStarAvailable < num)
                {
                    Service.gI().combine(1, vItemCombine);
                    Thread.Sleep(200);
                    item = vItemCombine.a.Cast<Item>().FirstOrDefault();
                }

                Code.StartUpgrade = false;
            })
            {
                IsBackground = true
            }.Start();
        ...
        if (chatTField.strChat.Equals(mResources.BUYS))
        {
            int num = 0;
            try
            {
            ...
            new Thread(() => {
                ...
                for (int i = 0; i < num; i++)
                {
                    Service.gI().buyItem((sbyte)(currItem.buyCoin > 0 ? 0 : 1), item.template.id, 0);
                    Thread.Sleep(50);
                }
            })
            {
